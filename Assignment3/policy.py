...
class AdaptivePolicy(object):
    ...
    def minUtilization(self):
        ...
        min_util_key = self.utilization.keys()[0]
        for coreSw in self.utilization.keys():
            value = self.utilization[coreSw]
            if (value < self.utilization[min_util_key]):
               min_util_key =  coreSw
        return min_util_key        
